...
<div class="bg-body p-3 rounded mb-3">
                ...
                <?php foreach ($groups as $groupName => $items): ?>
                    <optgroup label="<?= htmlspecialchars($groupName) ?>">
                        <?php foreach ($items as $k => $name): ?>
                            <option value="<?= htmlspecialchars($k) ?>" <?= ($selectedLog === $k) ? "selected" : "" ?>>
    ...
    <div class="p-3 rounded" style="background-color: #121417; min-height: 480px; max-height: 70vh; overflow-y: auto; font-family: SFMono-Regular, Consolas, 'Liberation Mono', Menlo, monospace; font-size: 0.85rem; line-height: 1.5;" id="logOutputContainer">
        <pre id="logOutput" class="mb-0 text-light" style="white-space: pre-wrap; word-break: break-all;"><?= htmlspecialchars($logContent) ?></pre>
    </div>
    <div class="d-flex justify-content-between align-items-center small text-muted pt-2">
        <span>Archivo activo: <strong><?= htmlspecialchars($availableLogs[$selectedLog]["name"] ?? $selectedLog) ?></strong></span>
