                    ...
                    <?php foreach ($domains as $d): ?>
                        <tr>
                            <td class="ps-3 fw-bold">
                                <a href="http://<?= htmlspecialchars($d["domain"]) ?>" target="_blank" class="text-decoration-none text-primary d-inline-flex align-items-center">
                            ...
                            <td>
                                <span class="badge bg-secondary-subtle text-secondary border border-secondary-subtle font-monospace">
                                    <?= htmlspecialchars($d["username"] ?? "admin") ?>
                                </span>
                            </td>
                            <td>
                                <form action="/web/update-php" method="POST" class="d-inline m-0">
                                    ...
                                    <select name="php_version" class="form-select form-select-sm d-inline-block" style="width: auto;" onchange="this.form.submit()">
                                        <?php foreach ($phpVersions as $php): ?>
                                            <option value="<?= htmlspecialchars($php["version"]) ?>" <?= ($d["php_version"] === $php["version"]) ? "selected" : "" ?>>
                            ...
                            <td>
                                <form action="/web/update-docroot" method="POST" class="d-inline-flex align-items-center m-0">
                                    <input type="hidden" name="domain_id" value="<?= (int)$d["id"] ?>">
                                    <input type="text" name="doc_root_suffix" class="form-control form-control-sm me-1 font-monospace" style="width: 140px;" value="<?= htmlspecialchars($d["doc_root_suffix"] ?? "public_html") ?>" title="Carpeta raiz">
                            ...
                            <td>
                                <?php if (!empty($d["ssl_enabled"])): ?>
                                    ...
                                    <a href="/web/disable-ssl/<?= (int)$d["id"] ?>" class="btn btn-sm btn-outline-danger py-0 px-1 ms-1" title="Desactivar SSL" onclick="return confirm('Desactivar SSL para <?= htmlspecialchars($d["domain"]) ?>?')">
                                        <i class="bi bi-x"></i>
                                    </a>
                                <?php else: ?>
                                    <a href="/web/enable-ssl/<?= (int)$d["id"] ?>" class="btn btn-sm btn-outline-primary text-uppercase fw-bold text-nowrap" onclick="return confirm('Generar e instalar certificado Let\'s Encrypt para <?= htmlspecialchars($d["domain"]) ?>?')">
                            ...
                            <td class="text-end pe-3 text-nowrap">
                                <div class="d-flex justify-content-end gap-1">
                                    ...
                                    <a href="/web/delete/<?= (int)$d["id"] ?>" class="btn btn-sm btn-outline-danger text-uppercase fw-bold text-nowrap" onclick="return confirm('Estas seguro de eliminar <?= htmlspecialchars($d["domain"]) ?>?')" title="Eliminar Dominio">
